...
trait Fnfunctor[F[-_,-_,+_]]
  extends FirstContravariant[F] with LeftContrvariant[F] with RightFunctor[F]
  with RightProfunctor[F] {

  def fnmap[E,A,R,EE,AA,RR](fa: F[E,A,R])(
    f: EE => E,
    g: AA => A,
    h: R => RR): F[EE,AA,RR]
  ...
  override def contramapLeft[E, A, R, AA](fa: F[E, A, R])(g: AA => A): F[E, AA, R] =
