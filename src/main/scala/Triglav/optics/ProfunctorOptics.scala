...
object ProfunctorOptics {
  ...
  def adapterC2P[S, T, A, B]: Adapter[S, T, A, B] => AdapterP[S, T, A, B] =
    iso =>
      new AdapterP[S, T, A, B] {
        override def apply[P[-_, +_]](implicit
            PP: Profunctor[P]
        ): Optics[P, S, T, A, B] =
