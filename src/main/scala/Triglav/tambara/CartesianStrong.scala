...
import Triglav.monoidal1.MonoidalCategoryInstances.productMCat.{
  α,
  α_inv,
  λ,
  λ_inv
...
//   private def unassoc[A,B,C]: ((A, (B,C))) => ((A,B), C) = α_inv
  ...
  def secondSecondIsDimapReAssoc[A, B, C, D](p: P[A, B]): Boolean = {
    ...
    val r2: P[(C, (D, A)), (C, (D, B))] =
      dimap[(C, (D, A)), (C, (D, B)), ((C, D), A), ((C, D), B)](r1)(α_inv, α)
...
//  def lunitr[A]: A => (Unit,A) = λ_inv
  ...
  def dimapLunitIsSecond[A, B, C, D](p: P[A, B]): Boolean = {
    ...
    // P[A,B] =============================> P[(Unit,A), (Unit,B)]
    val r: P[(Unit, A), (Unit, B)] =
      dimap[(Unit, A), (Unit, B), A, B](p)(λ, λ_inv)
...
object CartesianStrong {
  ...
  def uncurry[P[-_, +_], A, B, C](
      pa: P[A, B => C]
  )(implicit S: CartesianStrong[P]): P[(A, B), C] = {
