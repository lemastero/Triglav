...
object Limit {
  ...
  /** Given: object C and morphisms: A => C and B => C
    ...
    * g \/ \/ A ------> C f
    *
    * such that (universal property of pullback) for all other V, s: V => B, t:
    * V => A
    *
    * t V -------------+
    * | |
    ...
    * +---> A ------> C
    *
    * exists unique morphism h: V => U such that following diagrams commute:
    *
    * t V -------------+
    * | \ |
    * | \/ p \/
    ...
    * +---> A ------> C f
    *
    * Pullback is limit where diagram has 3 objects A, B, C and morphisms A => C
    * and B => C B
    * | \/ A ---> C
    */
