...
class BartoszMilewskiTalk {
  ...
  def yoEmbedF[G[_], H[_]]: YoEmbedF[G, H] = new YoEmbedF[G, H] {
    ...
    def from: ForAllF[G, H] => H ~> G = gh => gh(IdentityNat[G])
  }
  ...
  def ProReaderProfunctor[AA, BB]: Profunctor[ProReader[AA, BB, *, *]] =
    new Profunctor[ProReader[AA, BB, *, *]] {
      def dimap[S, T, A, B](
          sa: S => A,
          bt: B => T
