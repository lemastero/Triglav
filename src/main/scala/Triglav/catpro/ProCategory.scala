...
trait ProSemicategory[Morphism[_[_, _], _[_, _]]] {
  def compose[A[_, _], B[_, _], C[_, _]](f: Morphism[B, C])(
      g: Morphism[A, B]
  ): Morphism[A, C]
}
...
trait ProSemicategoryLaws[M[_[_, _], _[_, _]]] extends ProSemicategory[M] {
  ...
  def compositivityLaw[A[_, _], B[_, _], C[_, _], D[_, _]](
      g: M[B, C],
      f: M[A, B],
      h: M[C, D]
  ): Boolean = {
...
trait ProCategory[Morphism[_[_, _], _[_, _]]]
    extends ProSemicategory[Morphism] {
...
object ProCategoryInstances {
  trait DinatCat extends ProCategory[~~>] {
    ...
    override def compose[A[_, _], B[_, _], C[_, _]](f: B ~~> C)(
        g: A ~~> B
    ): A ~~> C = f compose g
  }
